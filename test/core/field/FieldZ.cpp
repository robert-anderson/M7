...
TEST(FieldZ, Test) {
    ...
    struct FermionOnvRowZ : RowZ {
        fieldsz::FermionOnv m_field;
        FermionOnvRowZ(size_t nsite): m_field(this, nsite){}
    };

    struct FermionOnvZ : fieldsz::FermionOnv {
        RowZ m_row;
        BufferedTableZ<FermionOnvRowZ> m_table;
        FermionOnvZ(size_t nsite):
        fieldsz::FermionOnv(nullptr, nsite),
        m_table("", {nsite}){
            static_cast<fieldsz::FermionOnv>(*this) = m_table.m_row.m_field;
            m_table.push_back();
        }


        using fieldsz::FermionOnv::to_string;
    };
