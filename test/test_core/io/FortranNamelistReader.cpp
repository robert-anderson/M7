...
TEST(FortranNamelistReader, FromFile) {
    ...
    uintv_t orbsym = {0, 2, 1, 5, 6, 4};
    ASSERT_EQ(integer::shifted(header.read_uints("ORBSYM"), false), orbsym);
    ASSERT_EQ(header.read_uints("ORBSYM"), integer::shifted(orbsym, true));
}