...
TEST(NumericCsvFileReader, ParseSingleReal){
    ...
    ASSERT_TRUE(parse::catching(line.cbegin()+nreal, line.cend(), inds));
    ASSERT_EQ(inds, chk_inds);
    v_t<double> real_values;
    v_t<double> chk_real_values = {-0.001368384767972724};
    ASSERT_TRUE(parse::catching(line.cbegin(), line.cbegin()+nreal, real_values));
    ...
    ASSERT_TRUE(parse::catching(line.cbegin(), line.cbegin()+nreal, complex_values));
