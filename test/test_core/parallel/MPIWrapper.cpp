...
TEST(MPIWrapper, Alltoall){
    defs::ivec_t send(mpi::nrank(), 0ul);
    defs::ivec_t recv(mpi::nrank(), 0ul);
...
TEST(MPIWrapper, Allgatherv){
    const size_t n=4;
    defs::ivec_t send(n, 0ul);
    defs::ivec_t recv(n * mpi::nrank(), 0ul);
    for (size_t i=0ul; i<n; ++i){
        send[i] = hash::in_range({mpi::irank(), i}, 3, 123);
    }
    defs::ivec_t recvcounts(mpi::nrank(), n);
    defs::ivec_t displs(mpi::nrank(), 0);
...
TEST(MPIWrapper, AllgathervRagged){
    ...
    defs::ivec_t send(nsend, 0ul);
    for (size_t i=0ul; i<nsend; ++i){
        send[i] = hash::in_range({mpi::irank(), i}, 3, 123);
    }

    defs::ivec_t recvcounts(mpi::nrank(), 0);
