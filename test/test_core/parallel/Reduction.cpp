...
TEST(Reduction, NdAllSum) {
    ...
    for (size_t irow = 0ul; irow < nrow; ++irow)
        for (size_t icol = 0ul; icol < ncol; ++icol)
            reduction.m_local[{irow, icol}] = hashing::in_range({irow, icol, mpi::irank()}, 0, 120);
    ...
    for (size_t irow = 0ul; irow < nrow; ++irow) {
        for (size_t icol = 0ul; icol < ncol; ++icol) {
            ...
            for (size_t irank = 0ul; irank < mpi::nrank(); ++irank)
                tot += hashing::in_range({irow, icol, irank}, 0, 120);
...
TEST(Reduction, NdAllSumSyndicate) {
    ...
    for (size_t irow = 0ul; irow < nrow; ++irow)
        for (size_t icol = 0ul; icol < ncol; ++icol)
            reduction2.m_local[{irow, icol}] = hashing::in_range({irow, icol, mpi::irank()}, 0, 150);
    ...
    for (size_t irow = 0ul; irow < nrow; ++irow) {
        for (size_t icol = 0ul; icol < ncol; ++icol) {
            size_t tot = 0ul;
            for (size_t irank = 0ul; irank < mpi::nrank(); ++irank)
                tot += hashing::in_range({irow, icol, irank}, 0, 150);
