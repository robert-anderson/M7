...
namespace conn_foreach_test {
    ...
    static defs::inds creatable_mode_indices(const field::BosOnv& mbf, size_t nboson_max) {
        defs::inds inds;
        for (size_t imode=0ul; imode<mbf.m_bd.m_nmode; ++imode) if (size_t(mbf[imode]) < nboson_max) inds.push_back(imode);
        return inds;
    }
...
TEST(ConnForeach, FrmGeneralEx1100) {
    ...
    conn_foreach::frm::General<1> foreach(FrmBasisData{nsite});
...
TEST(ConnForeach, FrmGeneralEx2200) {
    ...
    conn_foreach::frm::General<2> foreach(FrmBasisData{nsite});
...
TEST(ConnForeach, FrmMs2ConserveEx1100) {
    ...
    conn_foreach::frm::Ms2Conserve<1> foreach(FrmBasisData{nsite});
    ASSERT_EQ(foreach.m_exsig, exsig_utils::ex_single);
    foreach.loop_fn(mbf, fn);
...
TEST(ConnForeach, FrmMs2ConserveEx2200) {
    ...
    conn_foreach::frm::Ms2Conserve<2> foreach(FrmBasisData{nsite});
    ASSERT_EQ(foreach.m_exsig, exsig_utils::ex_double);
    foreach.loop_fn(mbf, fn);
...
TEST(ConnForeach, BosEx0001) {
    ...
    conn_foreach::bos::Ann foreach(BosBasisData{nmode});
...
TEST(ConnForeach, FrmBosEx1110) {
    ...
    conn_foreach_test::results_t frm_results;
    {
        ...
        frm::Ms2Conserve<1> foreach(FrmBasisData{nsite});
    ...
    for (size_t nboson_max =0ul; nboson_max < 6; ++nboson_max){
        ...
        frmbos::Product<frm::Ms2Conserve<1>, bos::Cre> foreach({FrmBasisData{nsite}}, {{nmode, nboson_max}});
