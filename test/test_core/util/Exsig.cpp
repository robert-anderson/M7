    ...
    for (uint_t ncref = 0ul; ncref <= c_nop_mask_frm; ++ncref) {
        for (uint_t nannf = 0ul; nannf <= c_nop_mask_frm; ++nannf) {
            for (uint_t ncreb = 0ul; ncreb <= c_nop_mask_bos; ++ncreb) {
                ...
                exsig = encode(ncref, nannf, ncreb, c_nop_mask_bos + 1);
                ASSERT_EQ(exsig, ~0ul);
            }
            exsig = encode(ncref, nannf, c_nop_mask_bos + 1, 0ul);
            ASSERT_EQ(exsig, ~0ul);
        }
        exsig = encode(ncref, c_nop_mask_frm + 1, 0ul, 0ul);
        ASSERT_EQ(exsig, ~0ul);
    }
    exsig = encode(c_nop_mask_frm + 1, 0ul, 0ul, 0ul);
    ASSERT_EQ(exsig, ~0ul);
}
