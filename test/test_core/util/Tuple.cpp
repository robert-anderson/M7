...
namespace tuple_test {
    ...
    struct PlusOneChkFn {
        ...
        void operator()(const str_t &v1, const str_t &v2) {
            auto v1_cpy = v1;
            v1_cpy.back()+=1;
            ASSERT_EQ(v1_cpy, v2);
        };
...
TEST(Tuple, ForeachSingle) {
    std::tuple<str_t, int, float> tup = {"example", 4, -1.5};
    std::tuple<str_t, int, float> tup_chk = {"examplf", 5, -0.5};
