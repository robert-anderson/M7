...
namespace frm_onv_connection_test {
    ...
    static size_t string_chk(const field::FrmOnv& src, const field::FrmOnv& dst, defs::ivec_t ann, defs::ivec_t cre){
        FrmOnvConnection connection(src);
        connection.connect(src, dst);
        return (connection.m_ann == ann) && (connection.m_cre == cre);
    }
    static size_t string_chk(const field::FrmOnv& src, const field::FrmOnv& dst, defs::ivec_t ann, defs::ivec_t cre, defs::ivec_t com){
        FrmOnvConnection connection(src);
