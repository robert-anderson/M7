...
TEST(HDF5Wrapper, FloatArray) {
    ...
    mpi::barrier();
    {
        ...
        auto v_read = hdf5::DatasetLoader::load_vector<float>(fr, "a_float_array", {false, true});
        auto v_def = v;
...
TEST(HDF5Wrapper, ComplexArray) {
    ...
    {
        hdf5::FileReader fr("table_test.h5");
        auto v_read = hdf5::DatasetLoader::load_vector<std::complex<float>>(fr, "a_complex_array", {false, true});
        auto v_def = v;
