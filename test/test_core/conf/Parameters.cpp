...
TEST(Parameters, ParsingYaml) {
    ...
    ASSERT_EQ(yf.get_as<defs::uintv_t>("section1.some_numbers"), defs::uintv_t({2, 3, 5, 1}));
...
TEST(Parameters, DefaultValues) {
    ...
    ASSERT_EQ(doc.m_section1.m_some_unspecified_numbers.get(), defs::uintv_t({3, 4, 6}));
...
TEST(Parameters, ParameterGroups) {
    ...
    ASSERT_EQ(doc.m_section1.m_some_numbers.get(), defs::uintv_t({2, 3, 5, 1}));
}