...
TEST(HeatBathSampler, AllExcitsGenerated) {
    ...
    ASSERT_EQ(std::accumulate(frequencies.begin(), frequencies.end(), 0ul), 962903);
    ASSERT_TRUE(std::all_of(frequencies.begin(), frequencies.end(), [](size_t i) { return i > 0; }));
}

TEST(HeatBathSampler, UnbiasedElecPair) {
    /*
    ...
    /*
    for (size_t p = 0ul; p < ham.nsite() * 2; ++p) {
        for (size_t q = 0ul; q < ham.nsite() * 2; ++q) {
            if (*weighted_frequencies.view(p, q) > 0)
                std::cout << p << " " << q << " " << *weighted_frequencies.view(p, q) / (iattempt + 1) - 1.0
                          << std::endl;
        }
    }
     */
...
TEST(HeatBathSampler, UnbiasedElecPairAndFirstVirtual) {
    /*
     * ensure that the ratio of generation frequency to proposal probability is uniform
     */
    AbInitioHamiltonian ham(defs::assets_root + "/DHF_Be_STO-3G/FCIDUMP");
