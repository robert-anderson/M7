...
class HashTable : public HashMap<key_T> {
    std::vector<std::pair<key_T, value_T>> m_pairs;
    std::stack<size_t> m_free;
public:

    HashTable(const size_t &nbucket) : HashMap<key_T>(nbucket), m_pairs(nbucket) {
    ...
    HashTable(HashTable &old, const size_t &nbucket) :
            HashMap<key_T>(old, nbucket), m_pairs(nbucket) {
