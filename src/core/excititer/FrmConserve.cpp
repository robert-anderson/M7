...
excititers::FrmConserve::convert(conn::FrmBosOnv &work_conn, const ExcitIter::fn_c_t<FrmBosOnv> &fn) {
...
void excititers::FrmConserve::foreach(const FrmOnv &src, conn::FrmOnv &conn, const ExcitIter::fn_c_t<FrmOnv> &body) {
    conn.clear();
    auto inner = [&](){
