...
#ifndef M7_NDARRAY_OLD_H
#define M7_NDARRAY_OLD_H

#include <cstddef>
#include "Indexer_old.h"

/*
 * NdArray_old is a fully and partially subscriptable multidimensional array
 ...
 * row of a matrix (NdArray_old<2>).
 */

template<typename T, size_t nind>
class NdArray_old {
protected:
    const Indexer_old<nind> m_indexer;
    std::vector<T> m_data_internal;
    T* m_data = nullptr;

public:
    template<typename ...Args>
    NdArray_old(const size_t &first, Args ...shape) : m_indexer(first, shape...),
                                                      m_data_internal(nelement(), 0), m_data(m_data_internal.data()) {}

    template<typename ...Args>
    NdArray_old(T* data, const std::array<size_t, nind> &shape) : m_indexer(shape), m_data(data){}
    ...
    template<typename ...Args>
    NdArray_old<T, nind - sizeof...(Args)> subarray(Args ...subs){
        std::array<size_t, nind-sizeof...(subs)> shape;
        std::copy(m_indexer.shape().begin()+sizeof...(subs),
            m_indexer.shape().end(), shape.begin());
        return NdArray_old<T, nind - sizeof...(subs)>(m_data + m_indexer.get_sub(subs...), shape);
    }

    void operator=(NdArray_old &src) {
