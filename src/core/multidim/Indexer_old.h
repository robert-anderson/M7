...
#ifndef M7_INDEXER_OLD_H
#define M7_INDEXER_OLD_H
...
public:
    ...
    Indexer_old(const size_t &first, Args ...shape) {
        static_assert(sizeof...(shape) + 1 == nind, "Invalid number of arguments.");
        set_shape(first, shape...);
        set_strides();
    }

    Indexer_old(const std::array<size_t, nind> &shape) {
        m_shape = shape;
        set_strides();
    }
