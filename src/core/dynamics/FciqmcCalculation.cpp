...
FciqmcCalculation::FciqmcCalculation(const InputOptions &input) :
    ...
    m_wf = std::make_unique<Wavefunction>(input, m_prop, reference);

    logger::write("Initializing FCIQMC Calculation...");
    logger::write("Reference determinant was detected to be: "+reference.to_string());
