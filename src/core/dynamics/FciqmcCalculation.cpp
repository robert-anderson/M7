...
FciqmcCalculation::FciqmcCalculation(const Options &opts) :
        m_opts(opts), m_ra(opts.nload_balance_block_per_rank * mpi::nrank(), 10),
        m_ham(opts), m_prop(m_ham, opts), m_wf(opts, m_ham.nsite(), m_ra) {
    ...
    Table::Loc ref_loc = {m_ra.get_rank(ref_onv), 0ul};
