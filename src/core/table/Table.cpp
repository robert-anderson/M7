...
char *Table_Z::begin(const size_t &irow) {
    ASSERT(irow<m_hwm)
    return begin() + irow * m_row_size;
}

size_t Table_Z::add_field(const TableField *field) {
...
void Table_Z::move(BufferWindow new_bw) {
...
void Table_Z::clear_row(const size_t &irow) {
    std::memset(begin(irow), 0, m_tight_row_size);
}

std::string Table_Z::field_details(size_t width) const {
    std::string res;
...
void Table_Z::print_field_details(size_t width) const {
    std::cout << field_details(width) << std::endl;
}

size_t Table_Z::bw_dsize() const {
    return m_bw.m_dsize;
}

void Table_Z::print_contents(const defs::inds *ordering) const {
...
void Table_Z::print_contents(const ExtremalValues &xv) const {
    defs::inds tmp;
