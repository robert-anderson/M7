...
bool ExcitGen::draw(const size_t &exsig, const FrmBosOnv &src, CachedOrbs &orbs,
                    defs::prob_t &prob, conn::FrmBosOnv &conn) {
    prob = 0.0;
    return false;
}

bool ExcitGen::draw(const size_t &exsig, const BosOnv &src, CachedOrbs &orbs,
                    defs::prob_t &prob, conn::BosOnv &conn) {
    prob = 0.0;
    return false;
}

bool ExcitGen::draw(const size_t &exsig, const BosOnv &src, CachedOrbs &orbs, defs::prob_t &prob, defs::ham_t &helem,
                    conn::BosOnv &conn) {
...
bool ExcitGen::draw(const size_t &exsig, const FrmBosOnv &src, CachedOrbs &orbs, defs::prob_t &prob, defs::ham_t &helem,
                    conn::FrmBosOnv &conn) {
    auto result = draw(exsig, src, orbs, prob, conn);
    if (!result) return false;
    helem = m_h.get_element(src, conn);
...
bool ExcitGen::draw(const size_t &exsig, const FrmOnv &src, CachedOrbs &orbs, defs::prob_t &prob, defs::ham_t &helem,
                    conn::FrmOnv &conn) {
    auto result = draw(exsig, src, orbs, prob, conn);
    if (!result) return false;
    helem = m_h.get_element(src, conn);
