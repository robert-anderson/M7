...
public:
    FermiBosHamiltonian(std::string fname, bool spin_major, size_t nboson_cutoff,
                        size_t nmode, defs::ham_t v, defs::ham_t omega) :
    ...
    size_t nci() const{
        return FermionHamiltonian::nci()*ci_utils::boson_dim(m_nmode, m_nboson_cutoff);
    }
...
public:
    defs::ham_t get_element(const conn::AsFermiBosOnv &afbconn) const {
        return FermionHamiltonian::get_element(afbconn.m_aconn)+
            get_element(afbconn.m_aconn, afbconn.m_bonvconn);
    }
