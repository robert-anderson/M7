...
FermionHamiltonian::FermionHamiltonian(size_t nelec, size_t nsite, bool complex_valued, defs::inds site_irreps):
...
defs::ham_t FermionHamiltonian::get_element(const field::FrmOnv &onv) const {
    return get_element_0000(onv);
}

defs::ham_comp_t FermionHamiltonian::get_energy(const field::FrmOnv &onv) const {
    return consts::real(get_element_0000(onv));
}

defs::ham_t FermionHamiltonian::get_element(const field::FrmOnv &onv, const conn::FrmOnv &conn) const {
    switch (conn.size()) {
        case 0:
            return get_element_0000(onv);
        case 2:
...
size_t FermionHamiltonian::nci() const {
