...
#ifndef M7_FCIITERS_H
#define M7_FCIITERS_H

#include "M7_lib/hamiltonian/Hamiltonian.h"
#include "M7_lib/foreach/MbfForeach.h"

/**
 * forms iterators over the FCI basis functions and pairs of basis functions that are appropriate for the given
 * Hamiltonian
 */
struct FciIters {
...
public:
     ...
     */
    static FciIters make(const Hamiltonian& h, sys::Particles particles, bool force_general);

    static FciIters make(const Hamiltonian& h);

    /**
     * fall-through case if no specific iterator is found or if general iterator is requested
     */
    static FciIters make_general(const Hamiltonian &h, sys::Particles particles);
};
