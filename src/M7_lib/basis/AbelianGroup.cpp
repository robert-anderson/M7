...
uint_t AbelianGroup::nirrep() const {
    return m_labels.size();
}

const uint_t &AbelianGroup::product(const uint_t &iirrep, const uint_t &jirrep) const {
...
AbelianGroup::AbelianGroup() : AbelianGroup({"A"}, [](const uint_t&, const uint_t&){return 0ul;}){}
...
AbelianGroup::AbelianGroup(const AbelianGroup &g1, const AbelianGroup &g2) :
        AbelianGroup(merge_labels(g1, g2),
                     [&](const uint_t &iirrep, const uint_t &jirrep) {
                         // combine the product tables of the two groups
                         const uint_t iirrep1 = iirrep / g2.nirrep();
                         const uint_t iirrep2 = iirrep - iirrep1 * g2.nirrep();
                         const uint_t jirrep1 = jirrep / g2.nirrep();
                         const uint_t jirrep2 = jirrep - jirrep1 * g2.nirrep();
...
PointGroup::PointGroup() : AbelianGroup({"0", "1", "2", "3", "4", "5", "6", "7"},
                                        [](const uint_t& i, const uint_t& j){return i^j;}){}

AbelianGroupMap::AbelianGroupMap(AbelianGroup grp, defs::uintv_t site_irreps) :
    ...
    REQUIRE_TRUE(std::all_of(m_site_irreps.cbegin(), m_site_irreps.cend(),
                       [&](uint_t i){return i<m_grp.nirrep();}), "irrep label OOB");
}

AbelianGroupMap::AbelianGroupMap(uint_t nsite) : AbelianGroupMap({}, defs::uintv_t(nsite, 0)){}
