...
namespace lattice {
    ...
    struct Lattice : Base {
        ...
        const topo_t m_topo;
    protected:
        static defs::uintv_t make_nadjs(const topo_t& topo) {
    ...
    std::shared_ptr<Base> make(std::string topo, defs::uintv_t site_shape, std::vector<int> bcs);
