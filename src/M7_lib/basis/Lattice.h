...
namespace lattice {
    ...
    struct OrthoTopology {
        ...
        OrthoTopology(const defs::ivec_t &shape, const std::vector<int> &bcs);

    private:
        int one_dim_phase(size_t iind, size_t jind, size_t idim) const;

    public:
        size_t isite_adj(const defs::ivec_t &inds, size_t idim, size_t value) const;
    ...
    struct Lattice : Base {
        ...
        const topo_t m_topo;
    protected:
        static defs::ivec_t make_nadjs(const topo_t& topo) {
    ...
    std::shared_ptr<Base> make(std::string topo, defs::ivec_t site_shape, std::vector<int> bcs);
