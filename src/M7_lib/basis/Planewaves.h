...
class Planewaves {
    ...
    mutable defs::ivec_t m_inds_work;


    static defs::ivec_t make_momentum_shape(const defs::ivec_t& wave_shape);
    static std::vector<std::vector<int>> make_momvecs(const defs::ivec_t& wave_shape);

public:
    static size_t size(const defs::ivec_t& wave_shape);
    static size_t size(size_t ndim, size_t nwave);

    explicit Planewaves(const defs::ivec_t& wave_shape);
