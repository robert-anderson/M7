...
namespace reduction {
    ...
    struct NdArray : ReductionBase<T> {
    ...
    private:
        NdArray(const NdFormat<nind>& /*format*/, tag::Int<true> /*zero_dims*/) :
                ReductionBase<T>(1) {
            m_local_ptr = &m_local;
            m_reduced_ptr = &m_reduced;
        }

        NdArray(const NdFormat<nind>& format, tag::Int<false> /*zero_dims*/) :
    ...
    public:
        NdArray(const uinta_t<nind>& shape) :
                NdArray({shape}, tag::Int<nind == 0>()){}

        NdArray() : NdArray({}, tag::Int<nind == 0>()){
    ...
    struct Syndicate {
        ...
        void add_member(ReductionBase<T>& member) {
            ...
            if (!m_groups[igroup])
                m_groups[igroup] = ptr::smart::make_unique<SyndicateGroup<T>>();
            dynamic_cast<SyndicateGroup<T> *>(m_groups[igroup].get())->add_member(member);
        }
