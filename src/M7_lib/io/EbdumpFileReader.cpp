...
size_t EbdumpFileReader::ranksig(const defs::ivec_t &inds) const {
...
size_t EbdumpFileReader::exsig(const defs::ivec_t &inds, const size_t &ranksig) const {
    DEBUG_ASSERT_EQ(inds.size(), 3ul, "incorrect maximum number of SQ operator indices");
    if (inds[1] == inds[2]) return exsig::ex_0010;
    else return exsig::ex_1110;
}

bool EbdumpFileReader::inds_in_range(const defs::ivec_t &inds) const {
    return inds[0] <= m_info.m_nmode && inds[1] <= m_norb_distinct && inds[2] <= m_norb_distinct;
}

void EbdumpFileReader::convert_inds(defs::ivec_t &inds) {
...
bool EbdumpFileReader::next(defs::ivec_t &inds, defs::ham_t &v) {
