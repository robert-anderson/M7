...
namespace hdf5 {
    ...
    static std::vector<hsize_t> convert_dims(const defs::inds_t &item_dims) {
    ...
    struct AttributeWriterBase {
        ...
        const defs::inds_t m_shape;
        const size_t m_nelement;
        hid_t m_memspace_handle;
        hid_t m_handle;

        AttributeWriterBase(hid_t parent_handle, std::string name, const defs::inds_t &shape, hid_t h5type);
    ...
    struct AttributeReaderBase {
        ...
        AttributeReaderBase(hid_t parent_handle, std::string name, const defs::inds_t &shape, hid_t h5type) :
    ...
    struct AttributeReader : AttributeReaderBase {
        AttributeReader(hid_t parent_handle, std::string name, const defs::inds_t &shape) :
    ...
    struct GroupWriter : GroupBase {
        ...
        template<typename T>
        typename std::enable_if<type_ind<T>() != ~0ul, void>::type
        save(std::string name, const T* v, const defs::inds_t& shape, std::vector<std::string> dim_labels={}, size_t irank=0ul) {
        ...
        template<typename T>
        typename std::enable_if<type_ind<T>() != ~0ul, void>::type
        save(std::string name, const std::complex<T>* v, const defs::inds_t& shape,
        ...
        template<typename T>
        typename std::enable_if<type_ind<T>() != ~0ul, void>::type
        save(std::string name, const std::vector<T>& v, const defs::inds_t& shape, std::vector<std::string> dim_labels={}, size_t irank=0ul){
    ...
    private:
        ...
        defs::inds_t get_dataset_shape(std::string name);
    ...
    public:
        ...
        template<typename T>
        typename std::enable_if<type_ind<T>() != ~0ul, void>::type
        load(std::string name, T* v, const defs::inds_t& shape){
            DEBUG_ASSERT_TRUE(child_exists(name), "Can't read from non-existent object");
        ...
        template<typename T>
        typename std::enable_if<type_ind<T>() != ~0ul, void>::type
        load(std::string name, std::complex<T>* v, const defs::inds_t& shape){
            DEBUG_ASSERT_TRUE(child_exists(name), "Can't read from non-existent object");
        ...
        template<typename T>
        typename std::enable_if<type_ind<T>() != ~0ul, void>::type
        load(std::string name, std::vector<T>& v, const defs::inds_t& shape){
            REQUIRE_EQ_ALL(v.size(), utils::nd::nelement(shape), "vector and shape are incompatible");
    ...
    struct NdDistListBase {
        ...
        NdDistListBase(hid_t parent_handle, std::string name, const defs::inds_t &item_dims, const size_t &nitem,
    ...
    private:
        NdDistListWriter(hid_t parent_handle, std::string name, const defs::inds_t &item_dims,
                         const size_t &nitem, hid_t h5type, const std::vector<std::string> &dim_labels = {});

    public:
        NdDistListWriter(FileWriter &parent, std::string name, const defs::inds_t &item_dims,
                         const size_t &nitem, hid_t h5type, const std::vector<std::string> &dim_labels = {}) :
                NdDistListWriter(parent.m_handle, name, item_dims, nitem, h5type, dim_labels) {}

        NdDistListWriter(GroupWriter &parent, std::string name, const defs::inds_t &item_dims,
                         const size_t &nitem, hid_t h5type, const std::vector<std::string> &dim_labels = {}) :
                NdDistListWriter(parent.m_handle, name, item_dims, nitem, h5type, dim_labels) {}
    ...
    struct NdDistListReader : NdDistListBase {
         ...
         *  global list shape
         */
        static defs::inds_t extract_list_dims(hid_t parent_handle, std::string name) {
         ...
         */
        static defs::inds_t extract_item_dims(hid_t parent_handle, std::string name) {
