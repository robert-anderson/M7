...
bool HamiltonianFileReader::next(defs::inds_t &inds, defs::ham_t &v) {
...
size_t HamiltonianFileReader::nset_ind(const defs::inds_t &inds) {
    return std::count_if(inds.begin(), inds.end(), [](const size_t &a) { return a != ~0ul; });
}

size_t HamiltonianFileReader::exsig(const defs::inds_t &inds) const {
    return exsig(inds, ranksig(inds));
}

void HamiltonianFileReader::decrement_inds(defs::inds_t &inds) {
