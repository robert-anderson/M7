...
bool NumericCsvFileReader::parsable_as(const std::string& str, int&) {
    for (auto c : str) {
        if (c=='-') continue;
...
bool NumericCsvFileReader::parsable_as(const std::string&, double&) {
    return true;
}

bool NumericCsvFileReader::parsable_as(const std::string&, float&) {
    return true;
}
