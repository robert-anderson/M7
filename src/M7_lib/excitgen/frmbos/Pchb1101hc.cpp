...
Pchb1101hc::Pchb1101hc(const FrmBosHam &h, PRNG &prng) :
        ...
        m_pick_n_given_pq(utils::pow<2>(h.m_hs.m_frm.m_sites.m_nspinorb), h.m_hs.m_bos.m_nmode) {
    const auto nmode = m_h.m_hs.m_bos.m_nmode;
    const auto nspinorb = m_h.m_hs.m_frm.m_sites.m_nspinorb;
...
bool Pchb1101hc::draw_frmbos(const size_t &exsig, const field::FrmBosOnv &src,
    ...
    const auto pq = p*m_h.m_hs.m_frm.m_sites.m_nspinorb+q;
    auto n = m_pick_n_given_pq.draw(pq, m_prng);
    conn.m_bos.clear();
    if (cre) {
        if (src.m_bos[n] == m_h.m_hs.m_bos.m_occ_cutoff) return false;
