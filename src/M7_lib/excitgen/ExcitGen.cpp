...
bool ExcitGen::draw_frm(OpSig /*exsig*/, const field::FrmOnv&, prob_t& prob, conn::FrmOnv& ) {
    prob = 0.0;
    return false;
}

bool ExcitGen::draw_frmbos(OpSig /*exsig*/, const field::FrmBosOnv&, prob_t& prob, conn::FrmBosOnv& ) {
    prob = 0.0;
    return false;
}

bool ExcitGen::draw_bos(OpSig /*exsig*/, const field::BosOnv&, prob_t& prob, conn::BosOnv& ) {
    prob = 0.0;
    return false;
}

bool ExcitGen::draw(OpSig exsig, const field::FrmOnv& src, prob_t& prob, ham_t& helem,
                    conn::FrmOnv& conn) {
    auto success = draw_h_frm(exsig, src, prob, helem, conn);
    return success && ham::is_significant(helem);
}

bool ExcitGen::draw(OpSig exsig, const field::FrmBosOnv& src, prob_t& prob, ham_t& helem,
                    conn::FrmBosOnv& conn) {
    auto success = draw_h_frmbos(exsig, src, prob, helem, conn);
    return success && ham::is_significant(helem);
}

bool ExcitGen::draw(OpSig exsig, const field::BosOnv& src, prob_t& prob, ham_t& helem,
                    conn::BosOnv& conn) {
