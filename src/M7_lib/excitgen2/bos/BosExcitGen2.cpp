...
#include "BosExcitGen2.h"

BosExcitGen2::BosExcitGen2(const BosHam &h, PRNG &prng, defs::inds exsigs, std::string description) :
...
bool BosExcitGen2::draw_frmbos(const size_t &exsig, const field::FrmBosOnv &src,
                               defs::prob_t &prob, conn::FrmBosOnv &conn) {
    return draw_bos(exsig, src.m_bos, prob, conn.m_bos);
}

bool BosExcitGen2::draw_h_frm(const size_t &exsig, const field::FrmOnv &src,
                              defs::prob_t &prob, defs::ham_t &helem, conn::FrmOnv &conn) {
    helem = 0.0;
    return false;
}

bool BosExcitGen2::draw_h_frmbos(const size_t &exsig, const field::FrmBosOnv &src,
...
bool BosExcitGen2::draw_h_bos(const size_t &exsig, const field::BosOnv &src, defs::prob_t &prob, defs::ham_t &helem,
                              conn::BosOnv &conn) {
