...
/*
 * void updater_fn (const views::FermionOnv&, defs::inds_t&)
 */
...
public:
    ...
    const defs::inds_t& inds_t() const{
        return m_inds;
    }
...
struct OccupiedUpdater {
    void operator()(const field::FrmOnv &mbf, defs::inds_t &inds_t);
};
...
struct NdOrbs__ {
    ...
    NdOrbs__(std::array<size_t, nind> shape, size_t nsite, const defs::inds_t& map):
    ...
    NdOrbs__(std::array<size_t, nind> shape, const field::FrmOnv &mbf, const defs::inds_t& map) :
            NdOrbs__(shape, mbf.m_bd.m_nsite, map) {
        update(mbf);
    }

    NdOrbs__(std::array<size_t, nind> shape, const field::FrmBosOnv &mbf, const defs::inds_t& map) :
    ...
    size_t size(const std::array<size_t, nind>& inds_t) const {
        return m_inds[m_format.flatten(inds_t)].size();
    }
    ...
    const defs::inds_t& operator[](const std::array<size_t, nind>& inds_t) const{
        return m_inds[m_format.flatten(inds_t)];
    }
...
struct NdOccupiedUpdater {
     ...
     */
    void operator()(const field::FrmOnv &mbf, const defs::inds_t& map,
...
class SpinNdOrbs__ : public NdOrbs__<updater_fn, 1> {
    defs::inds_t make_spin_map(size_t nsite) {
...
class SpinSymNdOrbs__ : public NdOrbs__<updater_fn, 2> {
    ...
    defs::inds_t make_map(const AbelianGroupMap& grp_map) {
        defs::inds_t out(2*grp_map.m_nsite, 0);
