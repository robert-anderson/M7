...
public:
     ...
     */
    defs::inds_t m_last_send_counts;
     ...
     * @param comm_nrow_est
     *  estimate of the number of rows per rank that will ultimately be required in the send buffer
     * @param exp_fac
     *  fractional over-allocation to make when resizing both tables
     * @param send
    ...
    void communicate() {
        ...
        defs::inds_t sendcounts(m_last_send_counts);
        for (auto &it: sendcounts) it *= row_size();
        defs::inds_t recvcounts(mpi::nrank(), 0ul);
        ...
        defs::inds_t recvdispls(mpi::nrank(), 0ul);
...
struct Communicator {
    ...
    struct DynamicRowSet : RankDynamic, RowProtector {
         ...
         */
        defs::inds_t m_counts;
        defs::inds_t m_displs;
        defs::inds_t m_ranks_with_any_rows;
    ...
    public:
        ...
        void before_block_transfer(const defs::inds_t &irows_send, size_t irank_send, size_t irank_recv) override {
