...
struct TableBase {
     ...
     */
    void clear_rows(const defs::inds_t &irows);
     ...
     * see RankAllocator.h
     */
    typedef std::function<void(const defs::inds_t &, size_t, size_t)> transfer_cb_t;
    /**
     ...
     */
    void transfer_rows(const defs::inds_t &irows, size_t irank_send, size_t irank_recv,
     ...
     */
    virtual std::string to_string(const defs::inds_t *ordering = nullptr) const;
