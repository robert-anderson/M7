...
conf::CiPmntr::CiPmntr(Group *parent) :
        ...
        m_c2_power_thresh(this, "c2_power_thresh", 2.0,
                          "power of the highest-magnitude C2 amplitudes defining the cutoff for permanitiator status"),
...
conf::Wavefunction::Wavefunction(Group *parent) :
        ...
        m_load_large_ci(this, "load_large_ci", "load set of MBFs with large CI weight expected and protect their rows from deletion",
                            "M7.large.h5", conf_components::Explicit),
...
conf::Shift::Shift(Group *parent) :
        ...
        m_nw_targets(this, "nw_targets", {10000.0},
                     "target walker numbers in each shift space, negative values are interpreted as a maximum float value"),
...
wf_comp_t conf::Shift::nw_target_total() const {
    wf_comp_t tot = 0.0;
    for (auto& v: m_nw_targets.m_value) tot+=std::abs(v);
    return tot;
}
