...
Rdm::Rdm(OpSig ranksig, OpSig indsig, sys::Sector sector, uint_t nvalue,
        ...
        m_stoch_thresh_contribs(stoch_thresh_contribs), m_neglect_tiny_contribs(neglect_tiny_contribs),
...
Rdm::Rdm(const conf::Rdms& opts, OpSig ranksig, OpSig indsig, sys::Sector sector,
        ...
        Rdm(ranksig, indsig, sector, nvalue, stoch_thresh_contribs, neglect_tiny_contribs, opts.m_distribution,
            // store sizing
 ...
 * helper for the ctor
 */
bool mag_opt_enabled(OpSig ranksig, double mag, const strv_t& ranks) {
    return (mag > 0.0) && bilinears::in_parsed_exsigs(ranksig, ranks);
}
