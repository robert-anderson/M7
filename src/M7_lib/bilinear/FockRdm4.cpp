...
void NonDiagFockRdm4::make_full_contrib(const RdmInds& full_inds, const OpSig& /*exsig*/, const wf_t& contrib, bool phase) {
    for (uint_t icre_contract = 0ul; icre_contract < 4ul; ++icre_contract){
        const uint_t icre = full_inds.m_frm.m_cre[icre_contract];
        const auto isite_cre = m_sector.m_frm.m_basis.isite(icre);
        const auto ispin_cre = m_sector.m_frm.m_basis.ispin(icre);
        for (uint_t iann_contract = 0ul; iann_contract < 4ul; ++iann_contract) {
            const uint_t iann = full_inds.m_frm.m_ann[iann_contract];
            const auto isite_ann = m_sector.m_frm.m_basis.isite(iann);
            const auto ispin_ann = m_sector.m_frm.m_basis.ispin(iann);
            ...
            for (uint_t iuncontract = 0ul; iuncontract < 4ul; ++iuncontract) {
                ...
                if (iuncontract!=icre_contract) {
                    ...
                    m_uncontracted_inds.m_frm.m_cre[i] = full_inds.m_frm.m_cre[iuncontract];
                }
                if (iuncontract!=iann_contract) {
                    i = iuncontract - (iuncontract > iann_contract ? 1 : 0);
                    m_uncontracted_inds.m_frm.m_ann[i] = full_inds.m_frm.m_ann[iuncontract];
                }
            }
...
void NonDiagFockRdm4::frm_make_contribs(const FrmOnv &src_onv, const conn::FrmOnv &conn,
    ...
    for (uint_t icomb = 0ul; icomb < promoter.m_ncomb; ++icomb) {
        ...
        NonDiagFockRdm4::make_full_contrib(m_full_inds, exsig, contrib, phase);
    }
}
...
void DiagFockRdm4::make_full_contrib(const RdmInds& full_inds, const OpSig& exsig, const wf_t& contrib, bool phase) {
    const auto exlvl = exsig.nfrm_cre();
    if (!exlvl) {
        /*
        ...
        for (uint_t icontract = 0ul; icontract < 4ul; ++icontract) {
            const uint_t iop = full_inds.m_frm.m_cre[icontract];
            REQUIRE_EQ(full_inds.m_frm.m_ann[icontract], iop, "not a diagonal contribution");
            const auto isite = m_sector.m_frm.m_basis.isite(iop);
            ...
            for (uint_t iuncontract = 0ul; iuncontract < 4ul; ++iuncontract) {
                ...
                if (iuncontract != icontract) {
                    ...
                    const uint_t op = full_inds.m_frm.m_cre[iuncontract];
...
void DiagFockRdm4::frm_make_contribs(const FrmOnv& src_onv, const conn::FrmOnv& conn,
    ...
    if (!exlvl) {
        for (uint_t icomb = 0ul; icomb < promoter.m_ncomb; ++icomb) {
             ...
             * connection and promotion-related Fermi phases are always false
             */
            DiagFockRdm4::make_full_contrib(m_full_inds, exsig, contrib, false);
        }
    }
    else {
        for (uint_t icomb = 0ul; icomb < promoter.m_ncomb; ++icomb) {
             ...
             */
            phase = phase ^ conn.phase(src_onv);
            DiagFockRdm4::make_full_contrib(m_full_inds, exsig, contrib, phase);
        }
    }
}