...
void ConnForeachGroup::log() const {
    ...
    for (auto& it : m_list) exsigs.emplace_back(utils::exsig::to_string(it->m_exsig));
    log::info("Connection iterator excitation signatures: {}", utils::convert::to_string(exsigs));
}
