...
mbf_foreach::Base::Base(BasisData bd, size_t niter) : m_bd(std::move(bd)), m_niter(niter) {}

void mbf_foreach::Base::loop(const mbf_foreach::Base::function_t<field::FrmOnv> &fn) { frm_loop(fn); }

void mbf_foreach::Base::loop(const mbf_foreach::Base::function_t<field::BosOnv> &fn) { bos_loop(fn); }

void mbf_foreach::Base::loop(const mbf_foreach::Base::function_t<field::FrmBosOnv> &fn) { frmbos_loop(fn); }
...
mbf_foreach::frm::Base::Base(size_t nsite, size_t niter) : mbf_foreach::Base({nsite, 0ul}, niter), m_mbf(nsite) {}
...
void mbf_foreach::frm::General::frm_loop(const std::function<void(const field::FrmOnv &)> &fn) { loop(fn); }
...
void mbf_foreach::frm::Spins::frm_loop(const std::function<void(const field::FrmOnv &)> &fn) { loop(fn); }
...
void mbf_foreach::frm::Ms2Conserve::frm_loop(const std::function<void(const field::FrmOnv &)> &fn) { loop(fn); }

mbf_foreach::bos::Base::Base(size_t nmode, size_t niter) : mbf_foreach::Base({0ul, nmode}, niter), m_mbf(nmode) {}
...
void mbf_foreach::bos::GeneralClosed::bos_loop(const std::function<void(const field::BosOnv &)> &fn) { loop(fn); }
...
void mbf_foreach::bos::GeneralOpen::bos_loop(const std::function<void(const field::BosOnv &)> &fn) { loop(fn); }

mbf_foreach::frm_bos::Base::Base(BasisData bd, size_t niter) : mbf_foreach::Base(bd, niter), m_mbf(bd) {}
