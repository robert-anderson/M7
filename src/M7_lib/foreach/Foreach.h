...
namespace foreach {
    ...
    namespace ctnd {
        ...
        public:
            ...
            void level_loop(cr_body_fn_t body, tags::Int<ilevel>) {
                constexpr size_t iind = ilevel - 1;
                auto &ind = Base<nind>::m_inds[iind];
                const auto &extent = m_shape[iind];
                for (ind = 0ul; ind < extent; ++ind) level_loop(body, tags::Int<ilevel + 1>());
            }

            void level_loop(cr_body_fn_t body, tags::Int<nind>) {
            ...
            void loop(cr_body_fn_t body, tags::Int<true>) {}

            void loop(cr_body_fn_t body, tags::Int<false>) {
                level_loop(body, tags::Int<1>());
            }

            void operator()(cr_body_fn_t body) override {
                loop(body, tags::Int<nind == 0>());
            }
        };
