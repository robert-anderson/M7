...
struct GlobalExtremalRows {
    ...
    GlobalExtremalRows(row_t &row, field::Numbers<T, nind> &field, bool largest, bool absval, defs::inds_t inds_to_cmp) :
    ...
    GlobalExtremalRows(row_t &row, field::Numbers<T, nind> &field, bool largest, bool absval, size_t ind_to_cmp) :
            GlobalExtremalRows(row, field, largest, absval, defs::inds_t{ind_to_cmp}){}
...
private:
    ...
    void sort(size_t nrow) {
        ...
        defs::inds_t ninclude_each_rank(mpi::nrank(), 0ul);
        if (mpi::i_am_root()) {
