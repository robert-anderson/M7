//
// Created by Robert J. Anderson on 04/08/2021.
//
...
bool LadderHolsteinCre::draw_frmbos(const size_t &exsig, const FrmBosOnv &src, CachedOrbs &orbs, defs::prob_t &prob,
                                     conn::FrmBosOnv &conn) {
    if (!m_h.m_bd.m_bos.m_nboson_max) return false;
    ...
    DEBUG_ASSERT_LE(curr_occ, m_h.nboson_max(), "current occupation of selected mode exceeds cutoff");

    // doubly-occupied sites are twice as likely to be drawn
    prob = src.m_frm.site_nocc(imode);
    prob /= occs.size();

    if (curr_occ == m_h.nboson_max()) return false;
...
bool LadderHolsteinAnn::draw_frmbos(const size_t &exsig, const FrmBosOnv &src, CachedOrbs &orbs, defs::prob_t &prob,
    ...
    DEBUG_ASSERT_LE(src.m_bos[imode], m_h.nboson_max(), "current occupation of selected mode exceeds cutoff");
