...
void mbf::set_from_def_array(field::FrmOnv &mbf, const std::vector<defs::uintv_t> &def, size_t idef) {
    if (def.empty()) return;
...
void mbf::set_from_def_array(field::BosOnv &mbf, const std::vector<defs::uintv_t> &def, size_t idef) {
    if (def.empty()) return;
    REQUIRE_LT(idef, def.size(), "MBF definition index OOB");
    mbf.zero();
    auto& definds = def[idef];
