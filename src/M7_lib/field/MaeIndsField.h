...
public:
    ...
    /**
     * can't simply copy ivec_t since the mev_ind_t is not the same as size_t in general, so must do a narrowing loop
     * @param inds
     *  indices in unsigned system words
     * @return
     *  reference to this
     */
    MaeIndsPartition &operator=(const defs::ivec_t &inds) {
        DEBUG_ASSERT_EQ(inds.size(), m_size, "ivec_t vector size incompatible");
        size_t iind = 0ul;
...
struct MaeIndsPair {
    ...
    MaeIndsPair &operator=(const std::pair<defs::ivec_t, defs::ivec_t> &inds) {
        m_cre = inds.first;
        m_ann = inds.second;
        return *this;
    }
...
public:
    ...
    void common_frm_inds(defs::ivec_t &common) const;
