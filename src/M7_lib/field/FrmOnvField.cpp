...
FrmOnvField &FrmOnvField::operator=(std::pair<const defs::inds_t &, const defs::inds_t &> setbits) {
    // prezero the element
    zero();
...
bool FrmOnvField::operator==(const defs::inds_t &inds) const {
...
int FrmOnvField::ms2() const {
    ...
    for (size_t idataword = 0; idataword < m_dsize; ++idataword) {
        ...
        while (work) {
            size_t ibit = idataword * base_t::nbit_dword() + utils::bit::next_setbit(work);
...
void FrmOnvField::set(const size_t &bit_offset, const defs::inds_t &setbits) {
    for(auto& i: setbits) set(bit_offset+i);
}

void FrmOnvField::set(const size_t &site_offset, const defs::inds_t &setbits_alpha, const defs::inds_t &setbits_beta) {
    for(auto& i: setbits_alpha) set({0, site_offset+i});
    for(auto& i: setbits_beta) set({1, site_offset+i});
}

void FrmOnvField::set(const defs::inds_t &setbits_alpha, const defs::inds_t &setbits_beta) {
    zero();
    for(auto& i: setbits_alpha) set({0, i});
    for(auto& i: setbits_beta) set({1, i});
}

void FrmOnvField::set_spins(const defs::inds_t &alpha_sites) {
...
void FrmOnvField::clr(const size_t &bit_offset, const defs::inds_t &clrbits) {
    for(auto& i: clrbits) clr(bit_offset+i);
}

void FrmOnvField::clr(const size_t &site_offset, const defs::inds_t &clrbits_alpha, const defs::inds_t &clrbits_beta) {
...
size_t FrmOnvField::get_beta_dataword(size_t idataword) const {
    ...
    if (iword_begin==iword_end){
        auto mask = utils::bit::make_range_mask<size_t>(ibit_in_word_begin, ibit_in_word_end);
        return (dptr[iword_begin] & mask) >> ibit_in_word_begin;
    }
    else {
        auto mask1 = utils::bit::make_range_mask<size_t>(ibit_in_word_begin, defs::nbit_word);
        auto mask2 = utils::bit::make_range_mask<size_t>(0, ibit_in_word_end);
